N_ROUNDS = 10

key = b'\xc3,\\\xa6\xb5\x80^\x0c\xdb\x8d\xa5z*\xb6\xfe\\'
ciphertext = b'\xd1O\x14j\xa4+O\xb6\xa1\xc4\x08B)\x8f\x12\xdd'

# S-Box substitution table
s_box = [
    
]

# Inverse S-Box substitution table
inv_s_box = [
    
]

def bytes2matrix(text):
    """
    Converts a 16-byte array into a 4x4 matrix.
    """
    return [list(text[i:i+4]) for i in range(0, len(text), 4)]

def matrix2bytes(matrix):
    """
    Converts a 4x4 matrix into a 16-byte array.
    """
    return bytes(matrix[i][j] for i in range(4) for j in range(4))

def add_round_key(state_matrix, round_key):
    """
    Adds the round key to the state matrix.
    """
    for i in range(4):
        for j in range(4):
            state_matrix[i][j] ^= round_key[i][j]

def sub_bytes(state_matrix):
    """
    Substitutes each byte in the state matrix with its corresponding value from the S-Box.
    """
    for i in range(4):
        for j in range(4):
            state_matrix[i][j] = s_box[state_matrix[i][j]]

def shift_rows(state_matrix):
    """
    Shifts the rows of the state matrix according to the AES algorithm.
    """
    state_matrix[1] = state_matrix[1][1:] + state_matrix[1][:1]
    state_matrix[2] = state_matrix[2][2:] + state_matrix[2][:2]
    state_matrix[3] = state_matrix[3][3:] + state_matrix[3][:3]

def mix_columns(state_matrix):
    """
    Mixes the columns of the state matrix according to the AES algorithm.
    """
    for i in range(4):
        s0 = state_matrix[0][i]
        s1 = state_matrix[1][i]
        s2 = state_matrix[2][i]
        s3 = state_matrix[3][i]

        state_matrix[0][i] = (
            gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3
        )
        state_matrix[1][i] = (
            s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3
        )
        state_matrix[2][i] = (
            s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3)
        )
        state_matrix[3][i] = (
            gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3)
        )

def inv_sub_bytes(state_matrix):
    """
    Substitutes each byte in the state matrix with its corresponding value from the inverse S-Box.
    """
    for i in range(4):
        for j in range(4):
            # Use inv_s_box directly without additional indexing
            state_matrix[i][j] = inv_s_box[state_matrix[i][j]]


def inv_shift_rows(state_matrix):
    """
    Shifts the rows of the state matrix in the inverse direction according to the AES algorithm.
    """
    state_matrix[1] = state_matrix[1][-1:] + state_matrix[1][:-1]
    state_matrix[2] = state_matrix[2][-2:] + state_matrix[2][:-2]
    state_matrix[3] = state_matrix[3][-3:] + state_matrix[3][:-3]

def inv_mix_columns(state_matrix):
    """
    Mixes the columns of the state matrix in the inverse direction according to the AES algorithm.
    """
    for i in range(4):
        s0 = state_matrix[0][i]
        s1 = state_matrix[1][i]
        s2 = state_matrix[2][i]
        s3 = state_matrix[3][i]

        state_matrix[0][i] = (
            gmul(0x0e, s0) ^ gmul(0x0b, s1) ^ gmul(0x0d, s2) ^ gmul(0x09, s3)
        )
        state_matrix[1][i] = (
            gmul(0x09, s0) ^ gmul(0x0e, s1) ^ gmul(0x0b, s2) ^ gmul(0x0d, s3)
        )
        state_matrix[2][i] = (
            gmul(0x0d, s0) ^ gmul(0x09, s1) ^ gmul(0x0e, s2) ^ gmul(0x0b, s3)
        )
        state_matrix[3][i] = (
            gmul(0x0b, s0) ^ gmul(0x0d, s1) ^ gmul(0x09, s2) ^ gmul(0x0e, s3)
        )

def expand_key(master_key):
    """
    Expands and returns a list of key matrices for the given master_key.
    """

    # Round constants https://en.wikipedia.org/wiki/AES_key_schedule#Round_constants
    r_con = (
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
        0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
        0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
        0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
    )

    # Initialize round keys with raw key material.
    key_columns = bytes2matrix(master_key)
    iteration_size = len(master_key) // 4

    # Each iteration has exactly as many columns as the key material.
    i = 1
    while len(key_columns) < (N_ROUNDS + 1) * 4:
        # Copy previous word.
        word = list(key_columns[-1])

        # Perform schedule_core once every "row".
        if len(key_columns) % iteration_size == 0:
            # Circular shift.
            word.append(word.pop(0))
            # Map to S-BOX.
            word = [s_box[b] for b in word]
            # XOR with first byte of R-CON, since the others bytes of R-CON are 0.
            word[0] ^= r_con[i]
            i += 1
        elif len(master_key) == 32 and len(key_columns) % iteration_size == 4:
            # Run word through S-box in the fourth iteration when using a
            # 256-bit key.
            word = [s_box[b] for b in word]

        # XOR with equivalent word from the previous iteration.
        word = bytes(i^j for i, j in zip(word, key_columns[-iteration_size]))
        key_columns.append(word)

    # Group key words into 4x4 byte matrices.
    return [key_columns[4*i : 4*(i+1)] for i in range(len(key_columns) // 4)]

def gmul(a, b):
    """
    Galois field multiplication of a and b.
    """
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set:
            a ^= 0x1b  # XOR with the irreducible polynomial x^8 + x^4 + x^3 + x + 1
        b >>= 1
    return p


def decrypt(key, ciphertext):
    round_keys = expand_key(key)  # Remember to start from the last round key and work backward through them when decrypting

    # Convert ciphertext to state matrix
    state_matrix = bytes2matrix(ciphertext)

    # Initial add round key step
    add_round_key(state_matrix, round_keys[-1])

    for i in range(N_ROUNDS - 1, 0, -1):
        inv_shift_rows(state_matrix)
        inv_sub_bytes(state_matrix)
        add_round_key(state_matrix, round_keys[i])
        if i != 0:
            inv_mix_columns(state_matrix)

    # Run the final round (skips the InvMixColumns step)
    inv_shift_rows(state_matrix)
    inv_sub_bytes(state_matrix)
    add_round_key(state_matrix, round_keys[0])

    # Convert state matrix to plaintext
    plaintext = matrix2bytes(state_matrix)

    return plaintext

print(decrypt(key, ciphertext))