...
def expand_key(master_key):
    ...
    while len(round_keys) < (N_ROUNDS + 1) * 4:
        word = list(round_keys[-1])

        if len(round_keys) % iteration_size == 0:
            word.append(word.pop(0))
            word = [s_box[b] for b in word]
            word[0] ^= r_con[i]
            i += 1
        elif len(master_key) == 32 and len(round_keys) % iteration_size == 4:
            word = [s_box[b] for b in word]

        word = bytes(i ^ j for i, j in zip(word, round_keys[-iteration_size]))
        round_keys.append(word)

    return [round_keys[4 * i: 4 * (i + 1)] for i in range(len(round_keys) // 4)]
